...
| A cost regression is invisible to every gate in `parity`: the anchor pins the NODE count, not what a node costs. `cargo xtask perf-budget` holds the instruction count to a recorded row. | [docs/10-tooling-ci.md](docs/10-tooling-ci.md) |
